...
#本段程序将每天的股票的数据的开盘价和收盘价的均值加入到数据库里
#注意,本程序每天只能执行一次,否则数据会出错
...
db_2 = conn.stock_data_ten_years_adjusted_divided_by_2_everyday_renew#一个新的数据库来装处理好的只有4个字段的表,代码,股票名,日期,平均价格
COLLECTION_NAME_FOUR='adjusted_4'
collection_names_list = db_2.collection_names()#现在这一步得到的是各个股票的代码,但是股票代码是小写的,所以要转化一下
collection_names_list_upper = [name.upper() for name in collection_names_list]#改成大写的代码,可以被取到大智慧里面



today = datetime.datetime.now()
end_time=str(today)[:10].replace('-','')
...
for name in collection_names_list_upper:
    # the_url='http://gw.yundzh.com/quote/kline?obj=%s&begin_time=%s-154033-223-8&end_time=%s-154033-223-8&field=ShiJian,KaiPanJia,ShouPanJia&period=1day&token='%(name,begin_time,end_time)
    the_url='http://gw.yundzh.com/quote/kline?obj=%s&begin_time=20170119-154033-223-8&end_time=20170619-154033-223-8&field=ShiJian,KaiPanJia,ShouPanJia&period=1day&split=1&token='%name
    # the_url='http://gw.yundzh.com/quote/kline?obj=%s&begin_time=20170606-154033-223-8&end_time=20170607-154033-223-8&field=ShiJian,KaiPanJia,ShouPanJia&period=1day&token='%name
