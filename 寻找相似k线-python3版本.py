...
#################################################################################################################################
#################################################################################################################################
tail_list,stock_name,tail_date_list = code_to_tail('sh600111')#得到了用户输入的股票的最近30个交易日的价格,股票名,最近的30个交易日的日期
#################################################################################################################################
#################################################################################################################################
#################################################################################################################################
#
...
for single_share in all_share_codes:
    ...
    for item in the_collection.find():
        ...
        if item['price_list_of_list'] == []:
            ...
            count+=1
            percent = round(((count/3165.0)*100),2)
...
def find_smallest_three():
    ...
    for single_s_list in s_list_each_stock_all:
        if single_s_list == []:
            print('%s list is empty！！！！！！！！'%single_s_list)
            continue
        else:
            single_s_list_sorted = sorted(single_s_list)
            # print(single_s_list)
...
print(name_need,list_need)#name_need是哪一只股票,list_need是需要的价格列表
