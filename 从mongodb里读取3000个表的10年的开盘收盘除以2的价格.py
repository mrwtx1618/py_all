#encoding:utf8
#本程序想要实现从mongodb里面读取3000个表的数据,取出收盘价和开盘价,然后除以2得到平均价格,装进数据库,每30个交易日形成一条曲线

import pymongo

conn = pymongo.MongoClient('localhost',27017) #创建连接
db_1 = conn.stock_data_ten_years_original #连接原始数据库来获取数据
db_2 = conn.stock_data_ten_years_adjusted_divided_by_2_everyday_renew #连接一个新的数据库来装处理好的只有4个字段的表
...
# print(dir(collection))
    for items in collection.find():
        ...
        avg_price_divided_2_list = []#前复权价列表
        the_date_list = []#日期列表
#         print(code,stock_brief)
        for i in range(data_number):
            i = str(i)#字典里面的取值是['4046']这种形式的,所以要把int转换成str类型
            open_price = (items[i]['开盘价'])
            close_price = (items[i]['收盘价'])
            avg_price_divided_2 = (open_price + close_price)/2
            # print(open_price)
            avg_price_divided_2_list.append(avg_price_divided_2)
            the_date = items[i]['交易日期']
            the_date_list.append(the_date)
        # print(the_date_list)
        # print(forward_adjusted_price_list)
        print(len(the_date_list))
        print(len(avg_price_divided_2_list))
        result_dict['code'] = code
        result_dict['name'] = stock_brief
        result_dict['date'] = the_date_list
        result_dict['avg_price_divided_2_price'] = avg_price_divided_2_list
        # result_dict_list.append(result_dict)
        print(result_dict)

        db_2['%s'%single_code].update_one({},{'$set':result_dict},upsert=True)
